...
class Aggregation(nn.Module):
    @abstractmethod
    def forward(
        self, M: Tensor, dest: Tensor, rev_index: Tensor, dim_size: int | None
    ) -> tuple[Tensor, Tensor]:
...
class Sum(Aggregation):
    def forward(self, M: Tensor, dest: Tensor, rev_index: Tensor, dim_size: int | None) -> Tensor:
...
class _AttentiveAggBase(Aggregation):
    def forward(self, M: Tensor, dest: Tensor, rev_index: Tensor, dim_size: int | None) -> Tensor:
        ...
        alpha = self._calc_weights(M, dest, rev_index)
...
class CompositeAggregation(Aggregation):
    ...
    def forward(self, M: Tensor, dest: Tensor, rev_index: Tensor, dim_size: int | None) -> Tensor:
        Ms, _ = zip(*[agg(M, dest, rev_index, dim_size) for agg in self.aggs])
